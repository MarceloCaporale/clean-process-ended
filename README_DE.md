...
**Ownership-first lokaler MCP Process Janitor fuer Coding-Agents; gebaut fuer Codex, Claude Code, Gemini CLI und MCP-kompatible Host-Workflows, in denen Subprozesse laenger leben koennen als die eigentliche Arbeit.**

`clean-process-ended` inspiziert lokale Subprozesse, die mit Agent- und MCP-Sessions zusammenhaengen, trennt Session-Ownership von schwachen Aehnlichkeitssignalen und erzeugt konservative Cleanup-Plaene im dry-run, bevor irgendeine Aktion am Environment erwogen wird.
...
- **Sichtbarkeit fuer Agent-Prozesse**: lokale Subprozesse zu Codex, Claude Code, Gemini CLI, generischen MCP-Hosts und kuenftig validierten Runtimes sehen, ohne Cleanup nach Prozessnamen zu verwenden.
...
Beta-Ergebnisse ersetzen keine menschliche Pruefung. Die oeffentliche CLI/MCP-Oberflaeche v0.7.2 ist diagnostisch und dry-run-orientiert; sie fuehrt keine echte Terminierung ueber die oeffentliche CLI/MCP-Oberflaeche aus.

## Validierungs-Snapshot

Die aktuelle v0.7.2 Public-Beta-Validierungsevidenz ist absichtlich als dry-run-Validierung formuliert, nicht als Validierung echter Cleanup-Ausfuehrung:
...
| Claude Code | Native lokale MCP-Validierung fuer v0.7.2 abgeschlossen; nur dry-run; sanitierte Evidenzzusammenfassung verfuegbar. |
| Gemini CLI | Native lokale MCP-Validierung fuer v0.7.2 abgeschlossen; nur dry-run; sanitierte Evidenzzusammenfassung verfuegbar. |
...
Diese Matrix beschreibt die oeffentliche Profilabsicht und den Validierungsstatus fuer v0.7.2. Sie behauptet keine Cleanup-Sicherheit ausserhalb der dokumentierten Policy-Gates. Siehe `docs/support-matrix.md` und `docs/validation/` fuer Evidenzlevel und Publikationsstatus.
...
`managed-cleanup-dryrun` ist in v0.7.2 nur report-only.
...
`codex-agent-mem` v1.0.1 und `clean-process-ended` v0.7.2 funktionieren jeweils eigenstaendig, sind aber als Ergaenzung gedacht. `codex-agent-mem` erhaelt Task-Kontinuitaet und Closure-State; `clean-process-ended` zeichnet Prozesshygiene-Evidenz und Janitor-Receipts im dry-run auf. Zusammen empfohlen:
...
Standardmaessig ist Cleanup dry-run und auf `owned_current_session` begrenzt. In v0.7.2 ist die oeffentliche CLI/MCP-Oberflaeche diagnostisch und dry-run-orientiert: reale Terminierung bleibt ueber oeffentliche CLI/MCP nicht operabel, weil Evidenz-Inputs absichtlich nicht exponiert werden. Das interne Real-Cleanup-Gate bleibt strenger als die oeffentliche Oberflaeche und verlangt vor jedem Terminierungspfad alle folgenden Punkte:
...
- `RELEASE_NOTES_v0.7.2.md`: Release Notes fuer v0.7.2.
...
Dieses Gate enthaelt Syntax-Checks, Tests, MCP-stdio-Smoke-Validierung, strikte Paketvalidierung, Dependency-Audit ab moderater Schwere, Public-Tree-Validierung, `npm pack --dry-run` und Smoke-Validierung des installierten Tarballs.

Danach `docs/release-checklist.md` abschliessen, Host-Evidenz aus `docs/verification/v0.7.2/README.md` aktualisieren, GitHub Actions abwarten, externe statische Audit-Pruefung von der oeffentlichen GitHub-URL ausfuehren und nur nach expliziter menschlicher Freigabe fortfahren.
