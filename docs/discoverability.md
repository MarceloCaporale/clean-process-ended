...
`0.7.3` is a public beta candidate for local-first, ownership-first process hygiene, runtime validation metrics and dry-run close-session checks for AI coding-agent workflows.

The public release should be framed as a runtime-validated process hygiene, evidence and close-task protocol tool, not an automatic process killer. Public v0.7.x CLI/MCP is dry-run-first; real cleanup evidence inputs are intentionally not exposed. Auto-cleanup remains disabled by default and experimental. The safe default workflow is `janitor_discovery`, `session_close_check`, report/dry-run review and optional storage of a sanitized `process_janitor_receipt` in `codex-agent-mem` v1.0.1.
...
- `clean-process-ended` v0.7.3: local-first process hygiene, dry-run janitor receipts and managed process evidence.
...
- local-first process hygiene
- AI agent process hygiene
